...
export default function AddressesTab() {
  ...
  const editAddress = async (id) => {
    ...
    if (addresses && addresses.length > 0) {
      const findAddress = addresses.find((item) => item.id === id);
      if (findAddress) {
        setFname(findAddress.name);
        setEmail(findAddress.email);
        setPhone(findAddress.phone);
        setAddress(findAddress.address);
        setHome(findAddress.type === "home");
        setOffice(findAddress.type === "office");
        setCountry(parseInt(findAddress.country_id));
        setState(parseInt(findAddress.state_id));
        setcity(parseInt(findAddress.city_id));
        setNewAddress(!newAddress);
        await getState(findAddress.country);
        await getcity(findAddress.country_state);
      }
    }
    // if (auth()) {
    //   apiRequest
    ...
    //           setHome(res.data.address.type === "home");
    //           setOffice(res.data.address.type === "office");
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex space-x-2.5 items-center">
                    <button
                      ...
                      type="button"
                      className="border border-qgray text-qgreen w-[34px] h-[34px] rounded-full flex justify-center items-center"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
